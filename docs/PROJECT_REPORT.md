...
Remaining limitations include a small manually maintained RAG corpus, incomplete school-specific programs, conservative rating coverage, no selected-year guarantee in address lookup, heuristic routing, model-dependent source relevance, and pending independent claim-level review. No chunking comparison or reranking improvement is claimed. The handout permits this custom scope; publication and demo artifacts must still be completed using [the demo guide](DEMO.md).
